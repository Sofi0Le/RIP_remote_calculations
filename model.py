...
class ApplicationForCalculation(models.Model):
    ...
    input_second_param = models.FloatField()
...
class ApplicationsCalculations(models.Model):
    application = models.OneToOneField(ApplicationForCalculation, models.DO_NOTHING, primary_key=True)  # The composite primary key (application_id, calculation_id) found, that is not supported. The first column is selected.
    calculation = models.ForeignKey('CalculationTypes', models.DO_NOTHING)
    result = models.FloatField(blank=True, null=True)

    class Meta:
        managed = False
        db_table = 'applications_calculations'
        unique_together = (('application', 'calculation'),)
...
class CalculationTypes(models.Model):
    ...
    calculation_image_url = models.CharField(max_length=50)
